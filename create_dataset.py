...
#initialize a dictionary to keep track of the number of each angle chosen so far, 
...
for (i, imagePath) in enumerate(imagePaths):
	...
	#if the image is None (meaning there was an issue loading the image from disk, 
	#simply skip it)
	if image is None:
		continue

	#rotate the image based on the selected angle, then construct the path to the base 
	#output directory
